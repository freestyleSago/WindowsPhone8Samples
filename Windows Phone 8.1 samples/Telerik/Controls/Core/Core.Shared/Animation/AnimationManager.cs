...
namespace Telerik.Core
{
    ...
    public static class RadAnimationManager
    {
        ...
        private async static void BeginInvokeCallback(Action callback, DependencyObject target)
        {
            if (callback != null)
            {
                await target.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
                    {
                        callback.Invoke();
                    });
