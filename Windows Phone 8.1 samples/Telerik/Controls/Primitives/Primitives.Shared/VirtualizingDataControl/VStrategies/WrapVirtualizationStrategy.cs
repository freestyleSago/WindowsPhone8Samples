...
namespace Telerik.UI.Xaml.Controls.Primitives
{
    internal class WrapVirtualizationStrategy : VirtualizationStrategy
    {
        ...
        internal int averageItemCountPerRow;
        ...
        internal override RadVirtualizingDataControlItem GetTopVisibleContainer()
        {
            ...
            int deltaFactor = Math.Round(this.GetItemRelativeOffset(topRow.firstItem), 1) > 0 ? -1 : 1;
        ...
        internal override double GetRealizedItemsBottom()
        {
            ...
            {
                case Orientation.Horizontal:
                    return Canvas.GetTop(this.owner.lastItemCache) + Canvas.GetTop(this.owner.itemsPanel) + lastRow.rowLength - this.owner.manipulationContainer.VerticalOffset;
                case Orientation.Vertical:
                    return Canvas.GetLeft(this.owner.itemsPanel) + Canvas.GetLeft(lastRow.firstItem) + lastRow.rowLength - this.owner.manipulationContainer.HorizontalOffset;
            }
            return 0;
        }
        ...
        internal override double GetRealizedItemsTop()
        {
            ...
            {
                case Orientation.Horizontal:
                    return Canvas.GetTop(this.owner.itemsPanel) + Canvas.GetTop(firstRow.firstItem) - this.owner.manipulationContainer.VerticalOffset;
                case Orientation.Vertical:
                    return Canvas.GetLeft(this.owner.itemsPanel) + Canvas.GetLeft(firstRow.firstItem) - this.owner.manipulationContainer.HorizontalOffset;
            }
            return 0;
        }
        ...
        internal override bool CanRecycleTop(double visibleItemsTop)
        {
            return this.wrapRows.Count > 1 &&
                   visibleItemsTop + this.firstWrapRow.rowLength < this.topVirtualizationThreshold;
        }
        ...
        {
            bool canFitVertically = visibleItemsBottom < this.ViewportLength + this.bottomVirtualizationThreshold;
        ...
        {
            return this.wrapRows.Count > 1 &&
                   visibleItemsBottom - this.lastWrapRow.rowLength > this.ViewportLength + this.bottomVirtualizationThreshold;
        }
        ...
        internal override void RecalculateViewportMeasurements()
        {
            ...
            this.averageItemExtent = Math.Max(1, this.allItemsExtent / this.owner.realizedItems.Count);
            this.averageItemCountPerRow = Math.Max(1, (int)(this.ViewportExtent / this.averageItemExtent));
            int itemCount = this.owner.GetItemCount();
            this.guessedRowCount = itemCount / this.averageItemCountPerRow;
            this.realizedItemsLength = this.lastWrapRow.rowOffset + this.lastWrapRow.rowLength - this.firstWrapRow.rowOffset;
            this.averageItemLength = this.realizedItemsLength / rowsCount;
            this.scrollableItemsLength = (this.averageItemLength * (this.guessedRowCount - rowsCount)) + this.realizedItemsLength;
        }
        ...
        internal override void ResetRealizationStartWhenLowerUIBufferRecycled(double position)
        {
            ...
            if (bottomDifference > this.bottomVirtualizationThreshold)
            {
                ...
                int itemCount = rowCount * this.averageItemCountPerRow;
        ...
        internal override void ResetRealizationStartWhenUpperUIBufferRecycled(double position)
        {
            ...
            if (topDifference < this.topVirtualizationThreshold)
            {
                ...
                int lastEvenIndex = lastRealizedDataItem.associatedDataItem.Index - ((lastRealizedDataItem.associatedDataItem.Index + 1) % this.averageItemCountPerRow) + 1;
                int currentLastItemIndex = this.owner.GetDataItemCount();

                if (lastRealizedDataItem != null)
                {
                    currentLastItemIndex = Math.Min(lastEvenIndex + itemCount, currentLastItemIndex - this.averageItemCountPerRow);
                }
