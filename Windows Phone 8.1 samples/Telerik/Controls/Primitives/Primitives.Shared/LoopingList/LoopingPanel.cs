...
namespace Telerik.UI.Xaml.Controls.Primitives.LoopingList
{
    ...
    public class LoopingPanel : Panel
    {
        ...
        /// <summary>
        /// Gets the <see cref="RadLoopingList"/> instance that hosts this panel.
        /// </summary>
        ...
        internal int TopLogicalIndex
        {
            ...
            set
            {
                if (this.topLogicalIndex == value)
                {
                    return;
                }

                if (value < 0 || value >= this.logicalCount)
                {
                    throw new IndexOutOfRangeException("TopLogicalIndex");
                }

                this.topLogicalIndex = value;
                this.BringIntoView(this.topLogicalIndex);
            }
        }
        ...
        internal void CenterItem(LoopingListItem item, bool select, bool animate)
        {
            if (select && this.owner != null)
            {
                ...
                {
                    this.ScrollToIndex(item, this.owner.SelectedIndex);
                    return;
                }
            }
            ...
            {
                this.BringIntoView(item.LogicalIndex);
            }
        }
        ...
        internal LoopingListItem ItemFromOffset(double offset)
        {
            foreach (LoopingListItem item in this.items)
            {
                ...
                if (itemOffset < offset && itemOffset + this.itemLength >= offset)
                {
                    return item;
                }
            }
        ...
        /// </summary>
        /// <param name="logicalIndex"></param>
        internal void BringIntoView(int logicalIndex)
        {
            this.BringIntoView(logicalIndex, false);
        }

        internal void UpdateLogicalIndexes(bool force, bool animate)
        {
            this.topLogicalIndex = this.GetFirstRealizedLogicalIndex();
            ...
            for (int i = 0; i < itemCount; i++)
            {
                ...
                {
                    this.owner.UpdateVisualItem(item, currentLogicalIndex, force, animate);
                }
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            if (this.isCentered && !this.centeringDuringLayout)
            {
                ...
                if (this.owner != null && this.owner.SelectedIndex != -1)
                {
                    this.BringIntoView(this.owner.SelectedIndex);
                }
                else
                {
        ...
        private void AnimateVerticalOffset(double to)
        {
        ...
        /// <param name="to">The final value of the animation.</param>
        private void AnimateVerticalOffset(Duration duration, EasingFunctionBase easing, double to)
        {
        ...
        private void BringIntoView(int logicalIndex, bool animate)
        {
            ...
            double offset = this.CalculateOffset(logicalIndex);
        ...
        private double CalculateOffset(int logicalIndex, bool negative)
        {
            double logicalWheelHeight = this.logicalCount * this.itemLength;
            double scrolledWheels = this.isLoopingEnabled ? (int)(this.visualOffset / logicalWheelHeight) * logicalWheelHeight : 0;

            double offset;
            if (negative)
            {
                offset = -(logicalIndex * this.itemLength) - scrolledWheels;
            }
            else
            {
                offset = logicalWheelHeight - (logicalIndex * this.itemLength) + scrolledWheels;
            }
        ...
        /// <param name="logicalIndex">The desired logical index.</param>
        /// <returns>The offset at which the index is completely visible.</returns>
        private double CalculateOffset(int logicalIndex)
        {
            bool backward;
            ...
            {
                backward = logicalIndex <= this.logicalCount / 2;
            }

            return this.CalculateOffset(logicalIndex, backward);
        }
        ...
        private void ScrollToIndex(LoopingListItem item, int to)
        {
            if (!this.IsInitialized())
            {
                return;
            ...
            if (item.LogicalIndex == to)
            {
                this.BringIntoView(to);
                return;
            }

            int indexOffset = Math.Abs(from - to);
            int pivotIndex = this.logicalCount / 2;
            int sign = Math.Sign(from - to);
        ...
        private void CreateVisualItems()
        {
            ...
            if (this.visualCount == 0 || this.visualCount == oldCount)
            {
                return;
            }

            if (this.visualCount != oldCount)
            {
                this.Children.Clear();
                this.visualIndexChain.Clear();
                this.items.Clear();
            ...
            {
                this.UpdateLogicalIndexes(this.visualCount != oldCount, false);
            }
            else
            {
                this.BringIntoView(this.topLogicalIndex);
        ...
        internal void UpdateWheelCore(double newOffset, bool force)
        {
            ...
            double logicalLength = this.logicalCount * this.itemLength;
            ...
            {
                topIndex = this.visualCount - hiddenItems - 1;
                bottomOffset = normalizedOffset;

                if ((this.visualOffset % logicalLength) == 0)
                {
                    // corner case
                    // TODO: think over for a better algorithm that eliminates the need of a corner case handling
                    topIndex++;
                }
            ...
            this.UpdateLogicalIndexes(force, false);
        }
        ...
        private double GetMaxOffset()
        {
            ...
            if (!this.isCentered && this.availableLength > logicalWheelLength)
            {
                return 0;
            }

            double maxOffset = this.availableLength - logicalWheelLength;
            maxOffset -= this.GetSnapOffsetWhenCentered();
        ...
        private int GetFirstRealizedLogicalIndex()
        {
            double logicalWheelLength = this.logicalCount * this.itemLength;

            double normalizedOffset = this.visualOffset % logicalWheelLength;
            int hiddenItems = (int)(Math.Abs(normalizedOffset) / this.itemLength);
            ...
            {
                index = this.logicalCount - hiddenItems - 1;
            }
        ...
        {
            double middleOffset = (this.availableLength - this.itemLength) / 2;

            middleOffset = this.GetSnapOffsetWhenCentered();
